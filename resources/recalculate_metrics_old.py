...
def sliding_window(arr, window_size, step_size):
    ...
    :param arr: 3D array with shape [time, joint, 3]z
...
def process_and_calculate_metrics_for_groups(grouped_files, directory):
    ...
    for suffix, files in grouped_files.items():
        ...
        # all_angle_error_m = { joint: np.median(angle_velocity_error[joint], axis=0)
        #                      for joint in angle_velocity_error.keys() }
        # all_angle_error_s = { joint: np.std(angle_velocity_error[joint], axis=0)
        #                      for joint in angle_velocity_error.keys() }
        ...
        #velocity_m, velocity_s, velocity_Qs = metrics.mean_velocity_error(aligned_gt, aligned_pred, sample_rate=25)
...
def main_load():
    ...
    with open('../statistics/multi/all_metrics.pkl', 'rb') as f:
        all_metrics = pickle.load(f)

    with open('../statistics/multi/keypoints_metrics.pkl', 'rb') as f:
        keypoints_metrics = pickle.load(f)

    with open('../statistics/multi/keypoints_metrics_single.pkl', 'rb') as f:
