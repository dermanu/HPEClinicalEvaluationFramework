...
def train(datapath: str, pars, rand, mode):
    ...
    train_loader = data.DataLoader(train, batch_size=config.BATCH_SIZE, num_workers=8, pin_memory=True, sampler=sampler)
    print(train_loader)
    test_loader = data.DataLoader(test, batch_size=config.BATCH_SIZE, num_workers=8, pin_memory=True, sampler=sampler_test)
